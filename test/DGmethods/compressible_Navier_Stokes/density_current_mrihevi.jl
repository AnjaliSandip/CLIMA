...
function run(
    ...
    rem = RemainderModel(model, (linmodel,))
    rdg = DGModel(
        rem,
        grid,
        Rusanov(),
    ...
    # Time step sizes (arbitrary for now)
    slow_dt = dt
    middle_dt = slow_dt / 10
    fast_dt = middle_dt / 10

    # Innermost 'fast' solver for horizontal acoustic waves
    fastsolver = LSRK144NiegemannDiehlBusch(hdg, Q; dt = fast_dt)

    # Middle solver for advection
    middlesolver = MRIGARKERK33aSandu(rdg, fastsolver, Q; dt = middle_dt)

    # Outer implicit solve for vertical acoustics
    linearsolver = GeneralizedMinimalResidual(Q; M = 10, rtol = 1e-10)
    ode_solver = MRIGARKIRK21aSandu(
        vdg,
